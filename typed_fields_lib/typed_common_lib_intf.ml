...
module type %{this n "S"} = sig
  ...
  module Type_ids %{each n "(T%i : T)"} : sig
    val type_id : (%{each n "T%i.t,"} 'a) t -> 'a Type_equal.Id.t
  end
  ...
  module Packed : sig
    ...
    [@@deriving compare, enumerate, equal, sexp] [@@unboxed]

    include Comparator.S with type t := t

    val pack : (%{each n "'t%i,"} 'a) field -> t
  end
end
...
module type S = sig
  ...
  module Packed : sig
    type 'a field := 'a t
    type t' = T : 'a field -> t'
    type t = { f : t' } [@@deriving compare, enumerate, equal, sexp] [@@unboxed]
...
module type S1 = sig
  ...
  module Packed : sig
    ...
    type t = { f : 't1. 't1 t' } [@@deriving compare, enumerate, equal, sexp] [@@unboxed]
