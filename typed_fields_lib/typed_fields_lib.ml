...
module Unit = struct
  ...
  let unreachable_code : type a. a t -> _ = function
    | (_ : a t) -> .
  ;;
...
struct
  ...
  let globalize (type r) (_ : r -> r) (t : r t) = globalize0 t
  let create { f } = f T
  let create_local { f } = f T
  ...
  module Packed_any = struct
    type 'r field = 'r t
    type t' = T : 'r. 'r field -> t' [@@unsafe_allow_any_mode_crossing]
    type t = { f : t' } [@@unboxed]
...
struct
  ...
  let globalize _1 (type r) (_ : r -> r) (t : ('t1, r) t) = globalize0 t
  let create { f } = f T
  let create_local { f } = f T
  ...
  module Packed_any = struct
    ...
    type 't1 t' = T : 't1 'r. ('t1, 'r) field -> 't1 t'
    [@@unsafe_allow_any_mode_crossing]
...
struct
  ...
  let globalize _1 _2 (type r) (_ : r -> r) (t : ('t1, 't2, r) t) = globalize0 t
  let create { f } = f T
  let create_local { f } = f T
  ...
  module Packed_any = struct
    ...
    type ('t1, 't2) t' = T : 't1 't2 'r. ('t1, 't2, 'r) field -> ('t1, 't2) t'
    [@@unsafe_allow_any_mode_crossing]
...
struct
  ...
  let globalize _1 _2 _3 (type r) (_ : r -> r) (t : ('t1, 't2, 't3, r) t) = globalize0 t
  let create { f } = f T
  let create_local { f } = f T
  ...
  module Packed_any = struct
    ...
    type ('t1, 't2, 't3) t' =
      | T : 't1 't2 't3 'r. ('t1, 't2, 't3, 'r) field -> ('t1, 't2, 't3) t'
    [@@unsafe_allow_any_mode_crossing]
...
struct
  ...
  let globalize _1 _2 _3 _4 (type r) (_ : r -> r) (t : ('t1, 't2, 't3, 't4, r) t) =
    globalize0 t
  ;;
  ...
  module Packed_any = struct
    ...
    type ('t1, 't2, 't3, 't4) t' =
      | T : 't1 't2 't3 't4 'r. ('t1, 't2, 't3, 't4, 'r) field -> ('t1, 't2, 't3, 't4) t'
    [@@unsafe_allow_any_mode_crossing]
...
struct
  ...
  let globalize _1 _2 _3 _4 _5 (type r) (_ : r -> r) (t : ('t1, 't2, 't3, 't4, 't5, r) t) =
    globalize0 t
  ;;
...
module S_of_S1 (M : S1) (T1 : T) :
  ...
  module Packed_any = struct
    ...
    let m_of_packed_any { f = T field } = M.Packed_any.pack field
    let m_of_packed_any__local { f = T field } = M.Packed_any.pack__local field
    let packed_any_of_m { M.Packed_any.f = T field } = { f = T field }
    let compare a b = M.Packed_any.compare (m_of_packed_any a) (m_of_packed_any b)
    ...
    let all = List.map M.Packed_any.all ~f:packed_any_of_m
    let sexp_of_t t = M.Packed_any.sexp_of_t (m_of_packed_any t)
    let sexp_of_t__stack t = M.Packed_any.sexp_of_t__stack (m_of_packed_any__local t)
    let t_of_sexp sexp = packed_any_of_m (M.Packed_any.t_of_sexp sexp)
    let globalize { f = T field } = { f = T (globalize0 field) }
    let pack (type a) (field : a field) = { f = T field }
    let pack__local (type a) (field : a field) = { f = T field }
