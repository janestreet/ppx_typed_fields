...
module Nothing = struct
  ...
  let globalize (type a) (_ : a -> a) (t : a t) = globalize0 t
...
struct
  ...
  let get (type r) (T : r t) (t : derived_on) : r option = Some t
  let create (type r) (T : r t) (t : r) : derived_on = t
  let which _ = { Packed.f = Packed.T T }
end
...
struct
  ...
  let get (type t1 r) (T : (t1, r) t) (t : t1 derived_on) : r option = Some t
  let create (type t1 r) (T : (t1, r) t) (t : r) : t1 derived_on = t
  let which _ = { Packed.f = Packed.T T }
end
...
struct
  ...
  let get (type t1 t2 r) (T : (t1, t2, r) t) (t : (t1, t2) derived_on) : r option = Some t
  let create (type t1 t2 r) (T : (t1, t2, r) t) (t : r) : (t1, t2) derived_on = t
  let which _ = { Packed.f = Packed.T T }
end
...
struct
  ...
  let get (type t1 t2 t3 r) (T : (t1, t2, t3, r) t) (t : (t1, t2, t3) derived_on)
    : r option
    =
    Some t
  ;;

  let create (type t1 t2 t3 r) (T : (t1, t2, t3, r) t) (t : r) : (t1, t2, t3) derived_on =
    t
  ;;
...
struct
  ...
  let create (type t1 t2 t3 t4 r) (T : (t1, t2, t3, t4, r) t) (t : r)
...
struct
  ...
  let create (type t1 t2 t3 t4 t5 r) (T : (t1, t2, t3, t4, t5, r) t) (t : r)
...
struct
  ...
  module Packed_any = struct
    ...
    let m_of_packed { f = T field } = M.Packed_any.pack field
    let m_of_packed__local { f = T field } = exclave_ M.Packed_any.pack__local field
    let packed_of_m { M.Packed_any.f = T field } = { f = T field }
    let compare a b = M.Packed_any.compare (m_of_packed a) (m_of_packed b)

    let compare__local a b =
      M.Packed_any.compare__local (m_of_packed__local a) (m_of_packed__local b) [@nontail]
    ;;

    let equal a b = M.Packed_any.equal (m_of_packed a) (m_of_packed b)

    let equal__local a b =
      M.Packed_any.equal__local (m_of_packed__local a) (m_of_packed__local b) [@nontail]
    ;;

    let hash_fold_t state t = M.Packed_any.hash_fold_t state (m_of_packed t)
    let hash t = Hash.of_fold hash_fold_t t
    let all = List.map M.Packed_any.all ~f:packed_of_m
    let sexp_of_t t = M.Packed_any.sexp_of_t (m_of_packed t)
    let sexp_of_t__stack t = exclave_ M.Packed_any.sexp_of_t__stack (m_of_packed__local t)
    let t_of_sexp sexp = packed_of_m (M.Packed_any.t_of_sexp sexp)
    let globalize { f = T field } = { f = T (globalize0 field) }
    let pack field = { f = T field }
    let pack__local field = exclave_ { f = T field }
...
module S_of_S1 (M : S1) (T1 : T) :
  ...
  module Packed_any = struct
    ...
    let m_of_packed { f = T field } = M.Packed_any.pack field
    let m_of_packed__local { f = T field } = M.Packed_any.pack__local field
    let packed_of_m { M.Packed_any.f = T field } = { f = T field }
    let compare a b = M.Packed_any.compare (m_of_packed a) (m_of_packed b)
    ...
    let all = List.map M.Packed_any.all ~f:packed_of_m
    let sexp_of_t t = M.Packed_any.sexp_of_t (m_of_packed t)
    let sexp_of_t__stack t = M.Packed_any.sexp_of_t__stack (m_of_packed__local t)
    let t_of_sexp sexp = packed_of_m (M.Packed_any.t_of_sexp sexp)
    let globalize { f = T field } = { f = T (globalize0 field) }
    let pack field = { f = T field }
    let pack__local field = { f = T field }
